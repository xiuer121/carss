...
@Results({ @Result(type = "redirectAction", location = "car-list"),
		@Result(name = "input", location = "car-info-update.jsp") })
public class CarUpdateAction extends ActionSupport {
	...
	public String show() {
		...
		this.carType = one.getCarType();
		this.condition = one.getCondition();
		this.travel = one.getTravel();
		this.discharge = one.getDischarge();
		this.creabox = one.getCreabox();
		this.driveMode = one.getDriveMode();
		this.color = one.getColor();
		this.used = one.getUsed();
		this.colorIn = one.getColorIn();
		this.procedures = one.getProcedures();
		this.seatNum = one.getSeatNum();
		this.chair = one.getChair();
		this.place = one.getPlace();
		this.fuel = one.getFuel();
		this.description = one.getDescription();
		this.originalPrice = one.getOriginalPrice();
		this.recState = one.getRecState();
		this.hotState = one.getHotState();
	...
	public void validateAdd() {
		 ...
		 //后缀名
		 this.ext =
		 this.fileFileName.substring(this.fileFileName.lastIndexOf('.')+1).toLowerCase();
		
		 List<String> arrowType =
		 Arrays.asList("image/gif","image/jpg","image/jpeg","image/pjpeg","image/png","image/bmp","image/x-png");
		 List<String> arrowExt =
		 Arrays.asList("gif","jpg","jpeg","png","bmp");
		
		 if(!(arrowType.contains(fileContentType.toLowerCase()) &&
		 arrowExt.contains(this.ext))){
		 this.addActionError("上传文件只接收gif/jpg/jpeg/png/bmp类型,您的文件类型为！"+fileContentType);
		 }
